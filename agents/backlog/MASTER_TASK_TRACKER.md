...
## Phase 11 — Executive Vendor Profile + DMZ Segregation (2026-06-04 →)

Owner asked for two related workstreams: (1) a per-vendor executive drill-down page so an executive can see vendor profile + award history + spend totals + win-rate, and (2) move the vendor portal into a DMZ-zone host while keeping admin/API/DB internal. Sequencing locked: dashboard first, DMZ after dashboard is verified.

- [x] BUG-102 — Department dashboard restructured to directory + per-dept drill-down (mirroring the BUG-100 vendor dashboard shape). Owner pushed back on BUG-101's single-page Department Overview. New endpoint `GET /api/v1/analytics/departments/:departmentId?year=YYYY|all` returning profile + metrics + full tender list + top vendors + multi-year spend trend + per-category breakdown. `/executive/departments` rewritten as compact directory (KPI strip + comparison bar + sortable clickable table). NEW `/executive/departments/[id]` detail page mirrors the vendor detail: header + 4 tabs (Overview / Tenders / Spend Trend / Vendors). Cross-navigation links between Executive, Vendors, Departments surfaces. No schema change, no migration, existing `executive:dashboard` perm.
  - Completed 2026-06-04. Key files: `apps/api/src/modules/analytics/{analytics.service.ts,analytics.controller.ts}`, `apps/web-admin/src/app/(admin)/executive/departments/{page.tsx,[id]/page.tsx}` (page.tsx rewritten, [id]/page.tsx new).

- [x] BUG-101 — Owner-walk bundle: (a) vendor self-registration form drops Registration Number / Tax Number / Country, adds Company Website (URL-validated); (b) CommercialMatrix + VendorComparisonCard tech-score now displays absolute units (e.g. "28 / 30") instead of percentage, matching the standalone Technical Comparison page; (c) `/executive` and `/executive/vendors` KPI strips colourised with a tone palette (blue/indigo/emerald/teal/green/amber/purple/cyan); (d) NEW `/executive/departments` dashboard — per-department year-scoped overview with tender count, estimated, awarded (BUG-088 fallback), savings, active pipeline, top-3 vendors. New endpoint `GET /api/v1/analytics/departments`, same `executive:dashboard` perm gate, no schema/migration changes. Verified 5 depts / 19 tenders / 115K awarded on staging.
  - Completed 2026-06-04. Key files: `apps/api/src/modules/analytics/{analytics.service.ts,analytics.controller.ts}`, `apps/api/src/modules/vendor-auth/{dto/vendor-register.dto.ts,vendor-auth.service.ts}`, `apps/web-admin/src/components/comparison/{CommercialMatrix.tsx,VendorComparisonCard.tsx}`, `apps/web-admin/src/app/(admin)/executive/{page.tsx,vendors/page.tsx,departments/page.tsx}` (new), `apps/web-admin/src/components/layout/Sidebar.tsx`, `apps/web-vendor/src/app/register/page.tsx`.

- [x] BUG-100 — Executive Vendor Profile + per-vendor history dashboard. New routes `/executive/vendors` (directory) and `/executive/vendors/[id]` (drill-down) on admin portal. Two new analytics endpoints with consolidated payloads (`GET /api/v1/analytics/vendors`, `GET /api/v1/analytics/vendors/:vendorId`) gated by existing `executive:dashboard` perm. BUG-088 fallback (Tender.awardedAmount → CommercialEvaluation.totalPrice) baked into every money calculation; verified end-to-end on staging (115,100 KWD lifetime spend matches manual SQL aggregate). Top Vendors rows on existing `/executive` page now link to drill-down. No schema change, no migration.
  - Completed 2026-06-04. Key files: `apps/api/src/modules/analytics/{analytics.service.ts,analytics.controller.ts}`, `apps/web-admin/src/app/(admin)/executive/vendors/{page.tsx,[id]/page.tsx}` (new), `apps/web-admin/src/components/layout/Sidebar.tsx`, `apps/web-admin/src/app/(admin)/executive/page.tsx`.

- [ ] DMZ Segregation — Move vendor portal to dedicated DMZ Ubuntu 22.04 + Docker 29.1.3 host with a Caddy reverse proxy serving `vn.hadiclinic.com.kw` (same-origin: `/` → web-vendor, `/api/v1/*` → internal API). Locked plan: Option A (reverse proxy + internal API stays private), staging-first, shadow-deploy then DNS flip. No code changes to NestJS API (vendor JWT split + `trust proxy` already in place). Detailed plan retained at `C:\Users\Administrator\.claude\plans\i-want-to-enhance-rustling-cerf.md` (follow-up section). Held pending owner verification of BUG-100 on staging.

- [x] BUG-115 — Negotiation workflow (multi-round, no deadline). New `Negotiation` tender state between `Commercial Evaluation / Comparison` and `Award Recommendation`. Procurement clicks Negotiate on Commercial Comparison to invite PASS vendors; invited vendors submit revised BoQ line prices + a new commercial PDF; comparison + AwardSummaryCard + Award Minutes PDF + Award History XLSX consume the negotiated price via a single resolver. Migration 032 adds 4 tables + `negotiation:launch` + `negotiation:view` perms. New backend module `apps/api/src/modules/negotiation/`. Frontend: `LaunchNegotiationDialog` + Negotiate button on `/commercial-comparison`; `VendorComparisonCard` renders stacked rounds with `% reduction`; vendor `NegotiationSection` lets invited vendors revise BoQ + upload PDF. Master plan §10 amended (locked-rule deviation, same pattern as BUG-114).
  - Completed 2026-06-09. Key files: `database/migrations/032_bug115_negotiation.sql`, `apps/api/prisma/schema.prisma`, `packages/shared-types/src/tender-status.ts`, `apps/api/src/modules/negotiation/{negotiation.module.ts,negotiation.service.ts,negotiation.controller.ts,negotiation-storage.service.ts,dto/*}` (new), `apps/api/src/modules/comparison/comparison.service.ts` (resolver + response shape), `apps/api/src/modules/analytics/analytics.service.ts` (resolver + `_resolveNegotiationSavings` + additive `negotiationSavings` on vendor profile + per-row on award history), `apps/api/src/modules/award/award.service.ts:{confirmAward,computeLowestPassBidId}` (auto-close + negotiation-aware lowest), `apps/api/src/modules/award/award-minutes.service.ts` (PDF savings sub-line), `apps/api/src/modules/reports/report-renderer.service.ts:awardHistory` (+2 columns), `apps/web-admin/src/app/(admin)/commercial-comparison/page.tsx` (Negotiate button + banner + dialog wiring), `apps/web-admin/src/components/comparison/{LaunchNegotiationDialog.tsx (new),VendorComparisonCard.tsx,AwardSummaryCard.tsx}`, `apps/web-vendor/src/components/bids/NegotiationSection.tsx` (new), `apps/web-vendor/src/app/(portal)/bids/[bidId]/page.tsx`, `docs/specs/IN_APP_COMPARISON_MASTER_PLAN_2026-05-27.md` §10 amendment block.

- [x] BUG-112 — Owner walkthrough 5-piece bundle: Tender Revert (new endpoint + dialog) · Tender search 400 fix (`q` → `search`) · Clarification reply private default reinforcement (DTO + visual) · Idle-timeout signout (JWT claim + activity hook + 401 interceptor on admin + vendor) · Vendor commercial PDF mandatory (BOQ exemption + Skip button removed). Migration 031 seeds `tender:revert` perm granted to SYSTEM_ADMIN + PROCUREMENT_ADMIN. New `POST /tenders/:id/revert` blocks when any bid is in `{SUBMITTED, LATE_SUBMITTED, LATE_ACCEPTED, EVALUATED, AWARDED, NOT_AWARDED, DISQUALIFIED}`. Reason ≥20 chars. HIGH-risk audit `TENDER_REVERTED`. Verified on staging: migration applied, containers rebuilt + restarted.
  - Completed 2026-06-07. Key files: `database/migrations/031_bug112_tender_revert.sql`, `apps/api/src/modules/tenders/{dto/revert-tender.dto.ts,tenders.service.ts,tenders.controller.ts}`, `apps/api/src/modules/clarifications/dto/reply-clarification.dto.ts`, `apps/api/src/modules/auth/auth.service.ts`, `apps/api/src/modules/vendor-auth/{vendor-auth.service.ts,vendor-auth.module.ts}`, `apps/web-admin/src/lib/{use-idle-timeout.ts,api.ts,auth.ts}`, `apps/web-admin/src/components/layout/IdleTimeoutGuard.tsx` (new), `apps/web-admin/src/app/(admin)/{layout.tsx,tenders/page.tsx,tenders/[id]/page.tsx,clarifications/page.tsx}`, `apps/web-admin/src/app/login/page.tsx`, `apps/web-admin/src/components/dialog/RevertTenderDialog.tsx` (new), `apps/web-vendor/src/lib/{use-idle-timeout.ts,auth.ts,api.ts}`, `apps/web-vendor/src/components/layout/PortalShell.tsx`, `apps/web-vendor/src/app/login/page.tsx`, `apps/web-vendor/src/app/(portal)/bids/wizard/[tenderId]/page.tsx`.
