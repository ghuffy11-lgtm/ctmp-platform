...
import type { DeptOverviewLabels } from './labels';
...
// 2026-08-13: parameterised so the Arabic page at /executive-ar/departments
// renders the same implementation with an Arabic label set and dir="rtl".
// English behaviour is unchanged — the defaults below reproduce it exactly.
export function DepartmentOverview({
  labels,
  dir = 'ltr',
  // When false (the Arabic page) nothing here may link out: every drill-down
  // target is an English-only screen, and /executive/tenders has no Arabic
  // equivalent. The KPI tiles used to ignore this and silently dumped an
  // Arabic reader into English — owner-reported 2026-08-21.
  interactive = true,
}: {
  labels: DeptOverviewLabels;
  dir?: 'ltr' | 'rtl';
  /** false in Arabic: the per-department profile page is English-only for now. */
  interactive?: boolean;
}) {
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-text-secondary mt-1">
            {labels.subtitle(data ? new Date(data.generatedAt).toLocaleDateString('en-GB') : '—')}
          </p>
        </div>
        <div className="flex items-center gap-2">
        ...
        <>
          ...
          {(() => {
            ...
            return (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <SummaryCard
                  label={labels.deptsActive}
                  value={String(totals.departmentCount)}
                  sub={labels.tendersCreated(totals.tenderCount)}
                  icon={Building2}
                  tone="blue"
                  href={interactive ? `/executive/tenders?createdYear=${year}&statusNot=Cancelled` : undefined}
                />
                <SummaryCard
                  label={labels.totalEstimated}
                  value={`${fmtKwd(totals.estimatedValue)} ${labels.currency}`}
                  sub={`${fmtFullKwd(totals.estimatedValue)} ${labels.currency}`}
                  icon={Wallet}
                  tone="indigo"
                  href={interactive ? `/executive/tenders?createdYear=${year}&statusNot=Cancelled&sort=estimatedBudget:desc` : undefined}
                />
                <SummaryCard
                  label={labels.totalAwarded}
                  value={`${fmtKwd(totals.awardedValue)} ${labels.currency}`}
                  sub={labels.awardsCount(totals.awardedCount)}
                  icon={Award}
                  tone="emerald"
                  href={interactive ? `/executive/tenders?awardedYear=${year}&hasAward=true&sort=awardedAmount:desc` : undefined}
                />
                <SummaryCard
                  label={labels.realisedSavings}
                  value={`${fmtKwd(totals.savings)} ${labels.currency}`}
                  sub={
                    totals.awardedCount > 0
                      ? labels.awardedOfBudget(fmtKwd(totals.awardedValue), fmtKwd(estimateOfAwarded))
                      : '—'
                  }
                  icon={TrendingDown}
                  tone="teal"
                  href={interactive ? `/executive/tenders?awardedYear=${year}&hasAward=true&sort=savings:desc` : undefined}
            ...
            <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
              ...
              <div className="flex items-center gap-4 text-xs text-text-secondary">
                <span className="flex items-center gap-1.5">
                  <span className="w-3 h-3 rounded bg-slate-300" />{labels.estimated}
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="w-3 h-3 rounded bg-emerald-500" />{labels.awarded}
                </span>
              </div>
            </div>
            {sortedRows.length === 0 ? (
                ...
                {sortedRows.map((row, i) => {
                  ...
                  return (
                    <li key={row.departmentId}>
                      <Link
                        href={`${labels.profileHrefBase}/${row.departmentId}?year=${year}`}
                        ...
                        <div className="flex items-center justify-between text-sm flex-wrap gap-2">
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <span className="font-semibold text-text-primary truncate">
                              {dir === 'rtl' ? (row.departmentNameAr?.trim() || row.departmentName) : row.departmentName}
                            </span>
                            <span className="text-[10px] font-mono text-text-secondary uppercase">{row.departmentCode}</span>
                            <span className="text-[10px] text-text-secondary">
                              {labels.tendersCount(row.tenderCount)}
                            </span>
                          </div>
                          <div className="text-right font-mono text-xs">
                            <p className="text-text-primary">{labels.estShort}: {fmtKwd(row.estimatedValue)}</p>
                            <p className={tone.text}>{labels.awdShort}: {fmtKwd(row.awardedValue)}</p>
                          </div>
                        </div>
          ...
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            <div className="px-5 py-3 border-b border-border bg-bg/30">
              <h2 className="text-base font-bold text-text-primary">{labels.allDepartments(year)}</h2>
              <p className="text-xs text-text-secondary mt-0.5">{labels.hint}</p>
            </div>
            {sortedRows.length === 0 ? (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border text-[10px] uppercase tracking-wider text-text-secondary">
                      <SortHeader k="departmentName" label={labels.colDepartment} />
                      <SortHeader k="tenderCount" label={labels.colTenders} align="right" />
                      <SortHeader k="awardedCount" label={labels.colAwarded} align="right" />
                      <SortHeader k="estimatedValue" label={labels.colEstimated} align="right" />
                      <SortHeader k="awardedValue" label={labels.colAwarded} align="right" />
                      <SortHeader k="savingsRate" label={labels.colSavingsPct} align="right" />
                      ...
                      return (
                        <tr key={row.departmentId} className="hover:bg-bg/40">
                          <td className="px-4 py-2.5">
                            {/* The per-department profile is English-only for
                                now, so Arabic renders the name as plain text
                                rather than sending a reader into it. */}
                            <Link
                              href={`${labels.profileHrefBase}/${row.departmentId}?year=${year}`}
                  ...
                  <tfoot className="bg-bg/40 font-semibold">
                    <tr>
                      <td className="px-4 py-2.5">{labels.totals}</td>
