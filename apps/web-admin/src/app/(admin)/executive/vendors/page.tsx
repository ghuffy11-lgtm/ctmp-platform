...
export default function ExecutiveVendorDirectoryPage() {
  ...
  const fetchDirectory = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to load vendor directory');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-text-primary tracking-tight">Vendor Directory</h1>
          </div>
          <p className="text-sm text-text-secondary mt-1">
            Executive view — every vendor with award history, lifetime spend, and win rate. Click any row for the full profile.
          </p>
        </div>
        <Link
          href="/executive"
      ...
      {data && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <KpiCard
            ...
            value={`${fmtKwd(data.kpis.lifetimeSpend)} KWD`}
            icon={Wallet}
            tone="indigo"
          />
          <KpiCard
      ...
      <div className="bg-card border border-border rounded-xl px-5 py-4 flex flex-wrap items-end gap-3">
        <div className="flex-1 min-w-[220px]">
          ...
          <div className="mt-1 relative">
            ...
            <input
              ...
              placeholder="Company name, registration #, tax #"
        ...
        <div>
          <label className="text-[10px] font-bold uppercase tracking-wider text-text-secondary">Award Year</label>
          <select
      ...
      {loading && !data && (
        <div className="bg-card border border-border rounded-xl p-10 flex items-center justify-center gap-2 text-text-secondary">
          <Loader2 className="w-4 h-4 animate-spin" /> Loading vendor directory…
        </div>
      )}
      ...
      {data && (
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          ...
          {data.rows.length === 0 ? (
            <p className="px-5 py-10 text-sm text-text-secondary text-center">No vendors match the current filters.</p>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border text-[10px] uppercase tracking-wider text-text-secondary">
                  ...
                  <SortHeader k="awardCount" label="Awards" align="right" />
                  <SortHeader k="totalAwarded" label="Total (KWD)" align="right" />
                  <SortHeader k="lastAwardedAt" label="Last Award" align="right" />
                  <SortHeader k="winRate" label="Win Rate" align="right" />
                ...
                {data.rows.map((r) => (
                  <tr key={r.vendorId} className="hover:bg-bg/40">
                    <td className="px-4 py-3">
                      <Link href={`/executive/vendors/${r.vendorId}`} className="text-text-primary font-medium hover:text-accent">
                        {r.companyName}
                      </Link>
                      {r.country && (
                    ...
                    <td className="px-4 py-3 text-right text-text-secondary">{fmtDate(r.lastAwardedAt)}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <Link
                        href={`/executive/vendors/${r.vendorId}`}
