...
// Ported from the admin portal 2026-08-21, unchanged in behaviour, so the two
// apps share one confirm/notify contract.
//
// Original rationale (BUG-078, 2026-06-01): browser-native confirm()/alert()
// popups are replaced by in-app modal dialogs. Owner directive — native popups
// feel like a "notification", not part of the app, and disrupt the workflow.
// The vendor portal never received that change and still had 2 confirm() and
// 5 alert() calls, including one on bid submission — the single most
// consequential, irreversible action a supplier takes. This closes that gap on
// the externally-facing portal.
//
// Two hooks:
...
function NotifyDialog({
  ...
  return (
    ...
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className={`bg-card border ${accent} rounded-3xl shadow-2xl max-w-md w-full overflow-hidden`}
      >
        <div className="px-6 py-4 border-b border-border">
