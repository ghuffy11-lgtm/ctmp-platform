...
|---|---|---|---|---|
| BUG-004 | High | Bug | Admin → Tender Detail documents | Technical Documents count shows 0 even though docs exist |
| BUG-005 | Medium | Bug | Admin → Tender Detail "Days Left" widget | Number is blank / shows "—" |
| BUG-008 | Medium | Bug | Admin → Tender Create form | Procurement Type field missing on create (only appears on edit) |
| BUG-009 | Medium | Bug | Admin → Tender Edit form | Department selector missing on edit (present on create) — only category shown |
| BUG-010 | High | Bug | Admin → Tender Create + Edit | Budget input missing on create; on edit budget is displayed but not editable |
| BUG-011 | High | Bug | Admin → Tender Edit (pre-approval) | PATCH rejected 400: "property category / procurementType / estimatedBudget should not exist" (DTO whitelist gap) |
| BUG-012 | High | Bug | Admin → Tender Detail (Draft) | Document upload button doesn't respond — can't attach RFQ docs to a Draft tender |
| BUG-014 | Medium | Bug | Admin → Tender Detail → Description tab | No attachment view — uploaded RFQ docs don't appear here for review |
...
| BUG-023 | High | Bug | Admin → Committee & Commercial | "Technically Qualified Vendors" → Commercial Envelopes section shows no documents |
| BUG-025 | High | Bug | Admin → Commercial Comparison | Commercial documents not available / not attached to comparison rows |
| BUG-026 | High | Feature | Admin → Commercial Comparison | Award recommendation forced to lowest price; committee must be able to pick any vendor with written justification |
| BUG-028 | Critical | Feature | Admin RBAC enforcement | Sidebar shows all menu items regardless of role; should restrict by permissions + scope tender visibility to user's department |
...
| BUG-033 | Medium | Bug | Admin → Commercial Comparison → Export | 2026-05-26 | Frontend used `commercial-comparison` (hyphen) + missing tenderId. Now uses `commercial_comparison` + sends tenderId. Backend DTO extended with `tenderId` + defensive `.toLowerCase()` on code lookup. End-to-end verified: 6,723-byte XLSX downloaded. |
